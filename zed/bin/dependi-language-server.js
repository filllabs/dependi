...
// ../../vscode/src/api/indexes/goEnv.ts
var DEFAULT_GO_PROXY = "https://proxy.golang.org";
function firstHttpGoProxy(goproxy = process.env.GOPROXY) {
...
function resolveGoIndexServerURL(configured, hasUserOverride) {
  if (hasUserOverride && configured) {
    return configured.replace(/\/$/, "");
  }
  return firstHttpGoProxy() || configured?.replace(/\/$/, "") || DEFAULT_GO_PROXY;
}
function isGoPrivateModule(modulePath, goprivate = process.env.GOPRIVATE, gonoproxy = process.env.GONOPROXY) {
  const patterns = [
  ...
  const path12 = modulePath.toLowerCase();
  return patterns.some((pattern) => matchGoPathPattern(path12, pattern.toLowerCase()));
}
function parseGoEnvList(value) {
  if (!value) {
    return [];
  }
  return value.split(",").map((s) => s.trim()).filter(Boolean);
}
function matchGoPathPattern(path12, pattern) {
  if (pattern.includes("*")) {
    const escaped = pattern.split("*").map((part) => part.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")).join(".*");
    return new RegExp(`^${escaped}(\\/|$)`).test(path12);
  }
  return path12 === pattern || path12.startsWith(`${pattern}/`);
}
...
var Settings = {
  ...
  load: function() {
    ...
    {
      const goIndexKey = "go.indexServerURL" /* GO_INDEX_SERVER_URL */;
      const goIndexInspect = config.inspect(goIndexKey);
      const hasGoIndexOverride = goIndexInspect?.globalValue !== void 0 || goIndexInspect?.workspaceValue !== void 0 || goIndexInspect?.workspaceFolderValue !== void 0;
      this.go.index = resolveGoIndexServerURL(
        config.get(goIndexKey),
        hasGoIndexOverride
      );
    }
...
function githubReleasesUrlFromGoModule(modulePath) {
  const path12 = modulePath.trim().replace(/^\/+/, "").replace(/\/+$/, "");
  if (!path12.toLowerCase().startsWith("github.com/")) {
    return void 0;
  }
      ...
      res.on(
        ...
        () => {
          ...
          try {
            info = {
              name,
              versions: response.packages[name].map((version) => version.version.replace(/^v/i, ""))
            };
          } catch (e) {
            reject(e);
          }
...
connection.onDidChangeConfiguration(async (change) => {
  const settings = change.settings && typeof change.settings === "object" ? change.settings : void 0;
  if (settings) {
    applyLspSettings(settings);
  } else {
    try {
