...
- **Same semver as VS Code (1.20.0).** Dependi keeps one version across `vscode/package.json`, `zed/extension.toml`, `zed/Cargo.toml`, and the bundled language server so both editors ship the same analysis engine.
- Use `./scripts/bump-version.sh <x.y.z>` to bump every manifest and rebuild `zed/bin/dependi-language-server.js`. Tag `v<x.y.z>` for a full release (VS Marketplace, OpenVSX, Zed); use `zed-v<x.y.z>` for a Zed-only republish.
- Zed packaging requires a version **greater than 1.11.0** (older third-party `dependi` builds remain in the blob store). Starting the shared line at **1.20.0** clears that floor without colliding with VS Code’s previous `1.5.0`.

### New Features

- First feature-complete Zed port of Dependi: latest versions, vulnerability hints, hover details, and update actions in the same manifests as VS Code.
...
- Go: Honors `GOPROXY` / `GOPRIVATE` / `GONOPROXY` when `dependi.go.indexServerURL` is unset. ([Issue #18](https://github.com/filllabs/dependi/issues/18))
- Go: Hover includes a GitHub **Releases** link for `github.com/...` modules. ([Issue #259](https://github.com/filllabs/dependi/issues/259))
...
- Rust/lockfiles: `silenceVersionOverflows` respected when a lockfile pins a version above the registry latest. ([Issue #221](https://github.com/filllabs/dependi/issues/221))
